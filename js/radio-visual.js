/* Extracted from app.js — radio-visual. Uses globals via globalThis (see app.js exposeVsGlobals). */
        class RadioVisualEngine {
            ...
            /**
             * Built-in phase offsets so L/R ribbons mirror correctly at slider 0°.
             * (Former Options defaults: L mid 342°, L low 310°.)
             */
            static get SPECTRUM_RIBBON_BASE_ROTATE_DEG() {
                return {
                    L: { high: 0, mid: 342, low: 310 },
                    R: { high: 0, mid: 0, low: 0 }
                };
            }
            static get SPECTRUM_RIBBON_SPIN_MIN() { return -200; }
            static get SPECTRUM_RIBBON_SPIN_MAX() { return 200; }
            static get SPECTRUM_RIBBON_SPIN_STEP() { return 10; }
            /** Preset used by Options “Animate spectrums”. */
            static get SPECTRUM_RIBBON_ANIMATE_SPIN() {
                return {
                    spinLHigh: 90,
                    spinLMid: -80,
                    spinLLow: 100,
            ...
            static clampSpectrumRibbonSpin(raw, fallback) {
                ...
                const src = (raw && typeof raw === 'object') ? raw : {};
                const min = RadioVisualEngine.SPECTRUM_RIBBON_SPIN_MIN;
                const max = RadioVisualEngine.SPECTRUM_RIBBON_SPIN_MAX;
                const step = RadioVisualEngine.SPECTRUM_RIBBON_SPIN_STEP;
            ...
            static migrateSpectrumRibbonRotateBase(raw) {
                const src = (raw && typeof raw === 'object') ? { ...raw } : {};
                if (src.ribbonRotateBaseV2) return src;
                const base = RadioVisualEngine.SPECTRUM_RIBBON_BASE_ROTATE_DEG;
                const norm = (deg) => {
                    let d = Number(deg);
                ...
                // Former Options defaults are now baked into SPECTRUM_RIBBON_BASE_ROTATE_DEG.
                src.rotateLMid = norm((Number(src.rotateLMid) || 0) - (base.L.mid || 0));
                src.rotateLLow = norm((Number(src.rotateLLow) || 0) - (base.L.low || 0));
                src.rotateLHigh = norm((Number(src.rotateLHigh) || 0) - (base.L.high || 0));
                src.rotateRMid = norm((Number(src.rotateRMid) || 0) - (base.R.mid || 0));
                src.rotateRLow = norm((Number(src.rotateRLow) || 0) - (base.R.low || 0));
                src.rotateRHigh = norm((Number(src.rotateRHigh) || 0) - (base.R.high || 0));
                src.ribbonRotateBaseV2 = 1;
                return src;
            }
            static loadSpectrumSettingsFromStorage() {
                try {
                    ...
                    if (!parsed || typeof parsed !== 'object') {
                        return { ...RadioVisualEngine.DEFAULT_SPECTRUM_SETTINGS, ribbonRotateBaseV2: 1 };
                    }
                    const migrated = RadioVisualEngine.migrateSpectrumRibbonRotateBase(parsed);
                    const next = RadioVisualEngine.clampSpectrumSettings(migrated);
                    next.ribbonRotateBaseV2 = 1;
                    if (!parsed.ribbonRotateBaseV2) {
                        try {
                            localStorage.setItem(
                                RadioVisualEngine.SPECTRUM_SETTINGS_KEY,
                                JSON.stringify(next)
                            );
                        } catch (_) {}
                    }
            ...
            static buildHubFsVizCycle() {
                const modes = globalThis.modes;
                const out = [];
                const keys = new Set();
                const push = (entry) => {
                    if (!entry || !entry.key || keys.has(entry.key)) return;
                    keys.add(entry.key);
                    out.push(entry);
                };
                if (Array.isArray(modes)) {
                    for (const m of modes) {
                        const name = m && m.name;
                        if (!name) continue;
                        if (RadioVisualEngine.isRadioModeName(name)) continue;
                        try {
                            if (globalThis.isRadioVisualModeName?.(name)) continue;
                        } catch (_) {}
                        if (name === 'DJ Decks' || name === 'Blank') continue;
                        if (/projectm|milkdrop/i.test(name) || /rainbow \(invert\)/i.test(name)) {
                            push({ kind: 'projectm', key: 'projectm', label: 'ProjectM' });
                            continue;
                        }
                        if (name === 'Logo Fx' || name === 'Logo') {
                            push({ kind: 'logofx', key: 'logofx-' + name, label: name === 'Logo Fx' ? 'LOGOFX' : 'Logo' });
                            continue;
                        }
                        if (m.sceneFn && typeof m.sceneFn === 'function') {
                            push({
                                kind: 'three',
                                key: 'three-' + name,
                                label: name,
                ...
                if (!keys.has('projectm')) {
                    out.unshift({ kind: 'projectm', key: 'projectm', label: 'ProjectM' });
                } else {
                    const i = out.findIndex((e) => e.kind === 'projectm');
                    if (i > 0) {
                ...
                return out.length
                    ? out
                    : [{ kind: 'projectm', key: 'projectm', label: 'ProjectM' }];
            }
            ...
            /** Toolbar centre strip: clock → volume → crossfade → auto-mix countdown → track remaining. */
            ...
            constructor(options = {}) {
                ...
                /** Last local track key that auto-switched the centre readout to remaining. */
                this._digitalToolbarRemainingAutoKey = '';
            ...
            _pauseDigitalHubDeckVideo() {
                [this.els.digitalHubDeckVideoA, this.els.digitalHubDeckVideoB].forEach((vid) => {
                    if (!vid) return;
            ...
            _syncDigitalHubDeckVideo(forceLoad) {
                ...
                const vidA = this.els.digitalHubDeckVideoA;
                const vidB = this.els.digitalHubDeckVideoB;
                if (!vidA || !vidB) return;
                try {
                    this._syncDualVideoCrossfade(vidA, vidB, '_digitalHubDeckVideoMode', forceLoad);
                } catch (_) {}
            }
            ...
            _setDigitalVideoLayer(vid, layer, op) {
                if (!vid) return;
                const o = Math.max(0, Math.min(1, Number(op) || 0));
                this._setDigitalStagingVideoOpacity(vid, o);
                if (o <= 0.001) return;
                if (!layer || !layer.url) return;
                if (layer.mode === 'idle' || layer.loop) {
                    this._applyDigitalIdleLogoVideo(vid, layer.url);
                    return;
                }
                if (typeof applyDeckBVideoPayloadToElement === 'function') {
                    applyDeckBVideoPayloadToElement(vid, layer, null);
                    return;
                }
                this._applyDigitalStagingVideoPayload(vid, layer);
            }

            _setDigitalStagingVideoLayer(vid, layer, op) {
                this._setDigitalVideoLayer(vid, layer, op);
            }

            _syncDualVideoCrossfade(vidA, vidB, modeKey, forceLoad) {
                if (!vidA || !vidB) return;
                if (forceLoad) this[modeKey] = '';
                const plan = (typeof computeDigitalStagingVideoCrossfadePlan === 'function')
                    ? computeDigitalStagingVideoCrossfadePlan()
                    : null;
                if (!plan) return;
                if (plan.bothIdle || !plan.dual) {
                    this[modeKey] = 'idle';
                    this._setDigitalVideoLayer(vidB, null, 0);
                    this._setDigitalVideoLayer(vidA, plan.layerA, 1);
                    return;
                }
                this[modeKey] = 'deck-dual';
                this._setDigitalVideoLayer(vidA, plan.layerA, plan.opA);
                this._setDigitalVideoLayer(vidB, plan.layerB, plan.opB);
            }

            _applyDigitalVideoCrossfadeOpacities(modeKey, vidA, vidB) {
                if (!vidA || !vidB || this[modeKey] !== 'deck-dual') return;
                try {
                    const plan = (typeof computeDigitalStagingVideoCrossfadePlan === 'function')
                        ? computeDigitalStagingVideoCrossfadePlan()
                        : null;
            ...
            _applyAllDigitalVideoCrossfadeOpacities() {
                this._applyDigitalVideoCrossfadeOpacities(
                    '_digitalStagingVideoMode',
                    this.els.digitalStagingVideoA,
                    this.els.digitalStagingVideoB
                );
                this._applyDigitalVideoCrossfadeOpacities(
                    '_digitalHubDeckVideoMode',
                    this.els.digitalHubDeckVideoA,
                    this.els.digitalHubDeckVideoB
                );
                this._applyDigitalVideoCrossfadeOpacities(
                    '_digitalDeckBVideoMode',
                    this.els.digitalDeckBVideoA,
                    this.els.digitalDeckBVideoB
                );
            }

            _applyDigitalStagingVideoCrossfadeOpacities() {
                this._applyAllDigitalVideoCrossfadeOpacities();
            }
            ...
            _syncDigitalStagingVideo(vidEl, forceLoad) {
                ...
                try {
                    this._syncDualVideoCrossfade(vidA, vidB, '_digitalStagingVideoMode', forceLoad);
                } catch (_) {}
            }
            ...
            async _deckTransportToolbarTap(deckKey) {
                ...
                const radioMode = (typeof isDeckInRadioMode === 'function') && isDeckInRadioMode(dk);
                ...
                if (radioMode && !isPlaying) {
                    ...
                    if (typeof playDeckOutput === 'function') playDeckOutput(dk);
                    else this._reconnectCurrentDeckStation(dk);
                } else if (hasSrc && !isPlaying) {
                    this._deckEngClearSuppress();
                    this._markCrossfadePauseExempt(dk);
                    try {
                        if (typeof releaseAutoMixDeferredLocal === 'function') {
            ...
            _setCrossfadeX(x) {
                ...
                try { this._applyAllDigitalVideoCrossfadeOpacities(); } catch (_) {}
            ...
            _deckPlaybackMediaForRemaining(deckKey) {
                const dk = deckKey === 'b' ? 'b' : 'a';
                try {
                    if (dk === 'a') {
                    ...
                    if (typeof getDeckBJogSeekMedia === 'function') return getDeckBJogSeekMedia();
                    if (typeof getDeckBRadioAudibleEl === 'function') return getDeckBRadioAudibleEl();
                    return (typeof audioElB !== 'undefined') ? audioElB : null;
                } catch (_) {}
                return null;
            ...
            _deckHasFiniteLocalDuration(deckKey) {
                const dk = deckKey === 'b' ? 'b' : 'a';
                try {
                    if (!state || !state.deckSourceMode || state.deckSourceMode[dk] !== 'local') return false;
                } catch (_) {
                    return false;
                }
                const media = this._deckPlaybackMediaForRemaining(dk);
                if (!media) return false;
                try {
                    const src = String(media.currentSrc || media.src || '');
                    if (!src || src === 'about:blank') return false;
                    const dur = Number(media.duration);
                    return Number.isFinite(dur) && dur > 0 && dur < 86400;
                } catch (_) {
                    return false;
                }
            }
            ...
            /**
             * Prefer the crossfader-audible deck when it has a local track with known duration;
             * otherwise use the other deck if it does.
             */
            _digitalTrackRemainingInfo() {
                const prefer = this._crossfaderAudibleDeckKey();
                const order = prefer === 'b' ? ['b', 'a'] : ['a', 'b'];
                for (let i = 0; i < order.length; i++) {
                    const dk = order[i];
                    if (!this._deckHasFiniteLocalDuration(dk)) continue;
                    const media = this._deckPlaybackMediaForRemaining(dk);
                    if (!media) continue;
                    try {
                        const dur = Number(media.duration);
                        ...
                        const src = String(media.currentSrc || media.src || '');
                        return { deck: dk, media, leftMs, key: `${dk}:${src}` };
                    } catch (_) {}
                }
                return null;
            }

            /** Track remaining: m:ss under one hour; h:mm when an hour or more remains. */
            _formatTrackRemainingReadout(ms) {
                if (!Number.isFinite(ms) || ms <= 0) return '0:00';
                const totalSec = Math.max(0, Math.ceil(ms / 1000));
                const h = Math.floor(totalSec / 3600);
                const m = Math.floor((totalSec % 3600) / 60);
            ...
            _crossfadedDeckPlaybackMedia() {
                const deck = this._crossfaderAudibleDeckKey();
                return this._deckPlaybackMediaForRemaining(deck);
            }

            _digitalTrackRemainingReadoutText() {
                const info = this._digitalTrackRemainingInfo();
                if (!info) return '—:—';
                return `-${this._formatTrackRemainingReadout(info.leftMs)}`;
            }

            _showDigitalToolbarTrackRemaining(deckKey) {
                if (this.skin !== 'digital') return;
                try {
                    if (deckKey === 'a' || deckKey === 'b') {
                        // Prefer the deck that just loaded when resolving remaining.
                        if (this._deckHasFiniteLocalDuration(deckKey)) {
                            const media = this._deckPlaybackMediaForRemaining(deckKey);
                            const src = media ? String(media.currentSrc || media.src || '') : '';
                            if (src) this._digitalToolbarRemainingAutoKey = `${deckKey}:${src}`;
                        }
                    }
                } catch (_) {}
                this._setDigitalToolbarCenterMode('remaining');
            }

            /** Called when a local track is loaded on a deck (play or arm). */
            notifyDigitalToolbarLocalTrack(deckKey) {
                if (this.skin !== 'digital') return;
                const dk = deckKey === 'b' ? 'b' : 'a';
                const media = this._deckPlaybackMediaForRemaining(dk);
                const tryShow = () => {
                    if (!this._deckHasFiniteLocalDuration(dk)) return false;
                    this._showDigitalToolbarTrackRemaining(dk);
                    return true;
                };
                if (tryShow()) return;
                if (!media) {
                    try { this._maybeAutoShowTrackRemaining(); } catch (_) {}
                    return;
                }
                const onMeta = () => {
                    try { media.removeEventListener('loadedmetadata', onMeta); } catch (_) {}
                    try { media.removeEventListener('durationchange', onMeta); } catch (_) {}
                    tryShow();
                };
                try { media.addEventListener('loadedmetadata', onMeta); } catch (_) {}
                try { media.addEventListener('durationchange', onMeta); } catch (_) {}
            }

            _maybeAutoShowTrackRemaining() {
                if (this.skin !== 'digital') return;
                if (this._digitalToolbarCenterMode === 'remaining') return;
                if (this._digitalToolbarCenterMode === 'automix') return;
                if (this._digitalToolbarCenterMode === 'crossfade') return;
                const info = this._digitalTrackRemainingInfo();
                if (!info) return;
                if (info.key === this._digitalToolbarRemainingAutoKey) return;
                this._digitalToolbarRemainingAutoKey = info.key;
                this._setDigitalToolbarCenterMode('remaining');
            }
            ...
            _syncDigitalToolbarCenterMode() {
                ...
                if (readout) {
                    const ariaLabels = {
                        ...
                        remaining: 'Track time remaining — tap for date and time'
                    };
            ...
            _cycleDigitalToolbarCenterMode() {
                // Remaining is a track overlay on the date/time slot — tap returns to the clock.
                if (this._digitalToolbarCenterMode === 'remaining') {
            ...
            _spectrumRibbonRotateDeg(side, bandKey) {
                ...
                const sideKey = side === 'R' ? 'R' : 'L';
                const band = String(bandKey || 'high').toLowerCase();
                const baseMap = RadioVisualEngine.SPECTRUM_RIBBON_BASE_ROTATE_DEG[sideKey] || {};
                const baseOff = Number(baseMap[band]) || 0;
                const user = Number(s[key]);
                const angle = Number.isFinite(user) ? Math.max(0, Math.min(360, user)) : 0;
                const total = baseOff + angle;
            ...
            _buildDigitalHubPanel() {
                ...
                const mkHubFsVizDock = () => {
                    const dock = document.createElement('div');
                    dock.className = 'radio-visual-digital-hub-fs-viz-dock';
                ...
                const mkHubVideoColumn = (innerWrap, columnKey) => {
                    const col = document.createElement('div');
                    col.className = 'radio-visual-digital-hub-video-column';
                    col.dataset.hubVideoColumn = columnKey;
                    const primary = document.createElement('div');
                    primary.className = 'radio-visual-digital-hub-video-primary';
                ...
                const deckVideoStack = document.createElement('div');
                deckVideoStack.className = 'radio-visual-digital-hub-deck-video-stack';
                const deckVideoA = document.createElement('video');
                deckVideoA.className = 'radio-visual-digital-hub-deck-video-el radio-visual-digital-hub-deck-video-el--a';
                const deckVideoB = document.createElement('video');
                deckVideoB.className = 'radio-visual-digital-hub-deck-video-el radio-visual-digital-hub-deck-video-el--b';
                [deckVideoA, deckVideoB].forEach((v) => {
                    v.muted = true;
                    v.playsInline = true;
                    v.setAttribute('playsinline', '');
                    v.preload = 'auto';
                });
                deckVideoStack.appendChild(deckVideoA);
                deckVideoStack.appendChild(deckVideoB);
                deckVideoWrap.appendChild(deckVideoStack);

                const digitalHubAiColumn = mkHubVideoColumn(aiWrap, 'ai');
                const digitalHubDeckVideoColumn = mkHubVideoColumn(deckVideoWrap, 'video');
                ...
                panel.append(volume, effects, digitalHubAiColumn, digitalHubDeckVideoColumn, bpm, off);
                return panel;
            }
            ...
            _toggleDigitalHubDeckVideoFullscreen() {
                ...
                try {
                    if (typeof toggleVideoSurfaceFullscreen === 'function') {
                        toggleVideoSurfaceFullscreen(stack, wrap);
                        return;
                    }
                } catch (_) {}
                try {
            ...
            _hubFsVizCycleList() {
                if (!this._hubFsVizCycleCache) {
                    this._hubFsVizCycleCache = RadioVisualEngine.buildHubFsVizCycle();
                }
                return this._hubFsVizCycleCache;
            }

            _activeHubVideoColumnEl() {
                const mode = this._digitalHubMode;
                if (mode === 'live' || mode === 'ai') return this.els.digitalHubAiColumn;
                if (mode === 'video') return this.els.digitalHubDeckVideoColumn;
                return null;
            }

            _activeHubFsVizMountEl() {
                const mode = this._digitalHubMode;
                if (mode === 'live' || mode === 'ai') return this.els.digitalHubFsVizMountAi;
                if (mode === 'video') return this.els.digitalHubFsVizMountVideo;
                return null;
            }
            ...
            _hubFsVizShouldShowDock() {
                if (!this._isDigitalPageFullscreen()) return false;
                const mode = this._digitalHubMode;
                if (mode !== 'live' && mode !== 'ai' && mode !== 'video') return false;
                return this._hubFsVizHasRoom();
            }

            _tearDownHubFsViz() {
                try {
                    if (this._hubFsVizAnimId) cancelAnimationFrame(this._hubFsVizAnimId);
                } catch (_) {}
                this._hubFsVizAnimId = null;
                try {
                    if (this._hubFsVizPmCycleTimeout) clearTimeout(this._hubFsVizPmCycleTimeout);
                } catch (_) {}
                this._hubFsVizPmCycleTimeout = null;
                try {
                    if (this._hubFsVizDetachInput) this._hubFsVizDetachInput();
                } catch (_) {}
                this._hubFsVizDetachInput = null;
                try {
                    const spin = this._hubFsVizScene
                        && this._hubFsVizScene.userData
                        && this._hubFsVizScene.userData.logoSpin;
                    if (spin && typeof spin.dispose === 'function') spin.dispose();
                } catch (_) {}
                try {
                    if (this._hubFsVizRenderer) this._hubFsVizRenderer.dispose();
                } catch (_) {}
                this._hubFsVizRenderer = null;
                this._hubFsVizScene = null;
                this._hubFsVizPmVisualizer = null;
                this._hubFsVizPmCanvas = null;
                this._hubFsVizResize = null;
                this._hubFsVizMountedKey = '';
            ...
            _mountHubFsVizProjectM(mount) {
                if (!mount || typeof butterchurn === 'undefined') return;
                try { initAudio(); } catch (_) {}
                if (!state || !state.audioCtx) return;
                const canvas = document.createElement('canvas');
                canvas.className = 'radio-visual-digital-hub-fs-viz-canvas';
                mount.appendChild(canvas);
                const resizePm = () => {
                    try {
                        const rect = mount.getBoundingClientRect();
                        ...
                        canvas.width = Math.floor(w * dpr);
                        canvas.height = Math.floor(h * dpr);
                        if (this._hubFsVizPmVisualizer && this._hubFsVizPmVisualizer.setRendererSize) {
                            this._hubFsVizPmVisualizer.setRendererSize(w, h);
                        }
                    } catch (_) {}
                };
                const rect0 = mount.getBoundingClientRect();
                ...
                if (!viz) return;
                this._hubFsVizPmVisualizer = viz;
                this._hubFsVizPmCanvas = canvas;
                try { viz.connectAudio(state.analyserNode); } catch (_) {}
                const presetMap = (typeof butterchurnPresets !== 'undefined' && butterchurnPresets.getPresets)
                    ? butterchurnPresets.getPresets() : {};
                const presetKeys = Object.keys(presetMap);
                let currentPresetIdx = Math.floor(Math.random() * Math.max(1, presetKeys.length));
                const loadPmPreset = (idx) => {
                    if (!presetKeys.length || !this._hubFsVizPmVisualizer) return;
                    try {
                        this._hubFsVizPmVisualizer.loadPreset(
                            presetMap[presetKeys[idx]],
                            Number(visualSettings.transitionSec) || 2.7
                        );
                    } catch (_) {}
                ...
                loadPmPreset(currentPresetIdx);
                this._hubFsVizResize = resizePm;
                resizePm();
                const loop = () => {
                    this._hubFsVizAnimId = requestAnimationFrame(loop);
                    try {
                        if (this._hubFsVizPmVisualizer) this._hubFsVizPmVisualizer.render();
                    } catch (_) {}
                };
                loop();
            }

            _mountHubFsVizLogoFx(mount) {
                if (!mount || typeof THREE === 'undefined' || typeof sceneLogoFx !== 'function') return;
                try { initAudio(); } catch (_) {}
                const scene = new THREE.Scene();
                const camera = new THREE.PerspectiveCamera(75, 1, 0.1, 1000);
                const renderer = new THREE.WebGLRenderer({ antialias: true, alpha: false });
                ...
                this._hubFsVizRenderer = renderer;
                this._hubFsVizScene = scene;
                this._hubFsVizResize = resizeLogoFx;
                resizeLogoFx();
                let lastLogoFxImgAspect = spin && spin.imgAspect ? spin.imgAspect : 0;
                const loop = () => {
                    this._hubFsVizAnimId = requestAnimationFrame(loop);
            ...
            _mountHubFsVizThree(mount, sceneFn) {
                if (!mount || typeof THREE === 'undefined' || typeof sceneFn !== 'function') return;
                try { initAudio(); } catch (_) {}
                const scene = new THREE.Scene();
                const camera = new THREE.PerspectiveCamera(75, 1, 0.1, 1000);
                camera.position.z = 8;
                ...
                mount.appendChild(canvas);
                let updateFn = null;
                try { updateFn = sceneFn(scene, camera); } catch (_) {}
                if (!updateFn) {
                    try { updateFn = sceneFn(scene, camera, null); } catch (_) {}
                }
                const resize = () => {
                    try {
                        const rect = mount.getBoundingClientRect();
            ...
            _mountHubFsVizCurrent() {
                const list = this._hubFsVizCycleList();
                const entry = list[this._hubFsVizIdx || 0];
                const mount = this._activeHubFsVizMountEl();
                if (!entry || !mount) return;
                if (this._hubFsVizMountedKey === entry.key) return;
                this._tearDownHubFsViz();
                this._hubFsVizMountedKey = entry.key;
            ...
            _syncHubFsVizDock() {
                const show = this._hubFsVizShouldShowDock();
                [this.els.digitalHubAiColumn, this.els.digitalHubDeckVideoColumn].forEach((col) => {
                    if (!col) return;
                    col.classList.toggle('is-fs-viz-dock-active', !!(show && col === this._activeHubVideoColumnEl()));
                    const dock = col.querySelector('.radio-visual-digital-hub-fs-viz-dock');
                    if (dock) dock.setAttribute('aria-hidden', show && col === this._activeHubVideoColumnEl() ? 'false' : 'true');
                });
                if (!show) {
                    this._tearDownHubFsViz();
                    return;
                ...
                this._mountHubFsVizCurrent();
                try {
                    if (this._hubFsVizResize) this._hubFsVizResize();
                } catch (_) {}
            }
            ...
            _tearDownDigitalDeckBPlayer() {
                ...
                this._digitalDeckBVideoSrc = '';
                this._digitalDeckBVideoMode = 'idle';
                [this.els.digitalDeckBVideoA, this.els.digitalDeckBVideoB].forEach((vid) => {
                    if (!vid) return;
                    try {
                        vid.pause();
                        vid.removeAttribute('src');
            ...
            _syncDigitalDeckBVideo(forceLoad) {
                const vidA = this.els.digitalDeckBVideoA;
                const vidB = this.els.digitalDeckBVideoB;
                if (!vidA || !vidB || this.digitalCenterMode !== 'deckB') return;
                try {
                    this._syncDualVideoCrossfade(vidA, vidB, '_digitalDeckBVideoMode', forceLoad);
                } catch (_) {}
            }
            ...
            _tickClock() {
                ...
                const mode = this._digitalToolbarCenterMode;
                if (mode === 'remaining') {
                    if (!this._digitalTrackRemainingInfo()) {
                        try { this._setDigitalToolbarCenterMode('clock'); } catch (_) {}
                    } else {
                        this._syncDigitalToolbarCenterReadout();
                    }
                } else if (mode === 'clock' || (mode === 'volume' && !this._digitalToolbarVolumePeekActive)
            ...
            _normalizeDigitalWheelDelta(ev) {
                let dx = Number(ev && ev.deltaX) || 0;
                let dy = Number(ev && ev.deltaY) || 0;
                if (ev && ev.deltaMode === 1) {
                    dx *= 16;
                    dy *= 16;
                } else if (ev && ev.deltaMode === 2) {
                    dx *= 800;
                    dy *= 800;
                }
                return { dx, dy };
            ...
            _onDigitalEqualiserCrossfadeWheel(ev) {
                ...
                if (Math.abs(dx) >= Math.abs(dy) && Math.abs(dx) > 0.5) {
                    raw = dx;
                } else if (ev.shiftKey && Math.abs(dy) > 0.5) {
                    raw = dy;
                } else {
                    return;
                }
                ...
                try { ev.preventDefault(); } catch (_) {}
                try { ev.stopPropagation(); } catch (_) {}
                this._setCrossfadeX(this._getCrossfadeX() + step);
                try { this._resumeDecksForCrossfade(); } catch (_) {}
                try {
            ...
            _bindDigitalStageInteractions(digitalCenterEl) {
                    ...
                    try {
                        if (RadioVisualEngine.digitalHubShowsDeckVideo(this._digitalHubMode) && ev.target && ev.target.closest) {
                            ...
                            if (hubVid) {
                                ...
                                if (stack && mount && typeof toggleVideoSurfaceFullscreen === 'function') {
                                    toggleVideoSurfaceFullscreen(stack, mount);
                                    return;
                                }
                            }
                        }
                ...
                digitalCenterEl.addEventListener('wheel', (ev) => {
                    this._onDigitalEqualiserCrossfadeWheel(ev);
                }, { signal: sig, passive: false, capture: true });
            }
            ...
            init() {
                ...
                let digitalDeckBVideoStack = null;
                let digitalDeckBVideoA = null;
                let digitalDeckBVideoB = null;
                ...
                digitalDeckBVideoStack = document.createElement('div');
                digitalDeckBVideoStack.className = 'radio-visual-digital-deck-b-video-stack';
                digitalDeckBVideoA = document.createElement('video');
                digitalDeckBVideoA.className = 'radio-visual-digital-deck-b-video radio-visual-digital-deck-b-video--a';
                digitalDeckBVideoB = document.createElement('video');
                digitalDeckBVideoB.className = 'radio-visual-digital-deck-b-video radio-visual-digital-deck-b-video--b';
                [digitalDeckBVideoA, digitalDeckBVideoB].forEach((v) => {
                    v.playsInline = true;
                    v.muted = true;
                });
                digitalDeckBVideoStack.appendChild(digitalDeckBVideoA);
                digitalDeckBVideoStack.appendChild(digitalDeckBVideoB);
                digitalDeckBVideo = digitalDeckBVideoStack;
                digitalDeckBMount.appendChild(digitalDeckBContent);
                digitalDeckBMount.appendChild(digitalDeckBVideoStack);
                try { this._wireDigitalDeckBVideoFullscreen(digitalDeckBVideoStack, digitalDeckBMount, sig); } catch (_) {}
                ...
                this.els = {
                    ...
                    digitalHubAiColumn: digitalHubPanel?.querySelector('[data-hub-video-column="ai"]'),
                ...
                try { this._wireHubFsVizDock(sig); } catch (_) {}
                ...
                try {
                    ...
                    if (this.skin === 'digital') {
                        ...
                        if (
                            ...
                            || this._digitalHubDeckVideoMode === 'deck-dual'
                            || this._digitalDeckBVideoMode === 'deck-dual'
                        ) {
                            this._applyAllDigitalVideoCrossfadeOpacities();
                        }
                        if (this._digitalStagingView === 'video') {
                            const now = performance.now();
                            const interval = (this._digitalStagingVideoMode === 'deck-dual' || this._digitalStagingVideoMode === 'idle')
                                ? 800
                                : 4000;
            ...
            destroy() {
                this._clearRadioAutoMixTimer();
                try { this._tearDownHubFsViz(); } catch (_) {}
                this._rvAutoMixCyclePending = false;
